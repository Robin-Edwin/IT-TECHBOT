...
def log_unknown(path, user_input):
    ...
    with open("unknown_questions.txt", "a") as f:
        if path:
            f.write(f"Unknownquestion: {' > '.join(path)} > {user_input_clean}\n")
        else:
            f.write(f"Unknownquestion: {user_input_clean}\n")
...
def get_bot_response(user_input):
    ...
    # ✅ Log unknown input at root level
    if matched_key is None and not path:
    ...
    if user_input_clean in current:
        ...
        if isinstance(next_node, dict):
            if "Answer" in next_node:
                ...
                return next_node["Answer"] + "\n\n  (Still not satisfied...? Please contact the Tech-Support.)", []

            ask = next_node.get("Ask")
            options = next_node.get("Options", [])
            return ask or "\n".join(f"- {k}" for k in next_node if k != "Ask"), options

        else:
            session.pop('path', None)
            return next_node + "\n\n  (Still not satisfied...? Please contact the Tech-Support.)", []

    # ✅ Log unknown input during flow
    log_unknown(path, user_input)
